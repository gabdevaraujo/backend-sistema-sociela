...
public class UsuarioResource {
	...
	public ResponseEntity<UsuarioDTO> create(@RequestBody UsuarioDTO item) {
		try {
			UsuarioDTO savedItem = service.save(item);
			return new ResponseEntity<UsuarioDTO>(savedItem, HttpStatus.CREATED);
		} catch (Exception e) {
	 ...
	 * delete(@PathVariable("id") entityIdType id) { try {
	 * repository.deleteById(id); return new
